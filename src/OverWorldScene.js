...
class OverWorldScene extends Phaser.Scene {
  ...
  create() {
    ...
    // Add the map 
    this.map = this.make.tilemap({ key: this.worldName });
    this.tileset = this.map.addTilesetImage('super-mario-16bit', 'tiles');

    console.log(this.tileset)

    // Dynamic layer because we want breakable and animated tiles
    this.groundLayer = this.map.createDynamicLayer('Tile Layer 1', this.tileset, 0, 0);
    ...
    this.projectiles = this.add.group({classType: Projectile, runChildUpdate: true, maxSize: 10})
    ...
    this.keys = {
      ...
      shoot: this.input.keyboard.addKey(Phaser.Input.Keyboard.KeyCodes.A)
    };
